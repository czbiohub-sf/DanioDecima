...
def load_trajectory_data(results_dir):
    ...
    trajectory_data = {}
    trajectory_files = list(results_dir.glob("*_simple_trajectory.csv"))
...
def create_motif_occurrence_tables(motifs_df):
    """Create motif occurrence tables for each model with enhanced statistics."""
...
# Keep the original functions but add calls to new ones
...
def create_volcano_plot_instructions(averaged_table, output_dir):
    """Create instructions for using the averaged normalized table with volcano plots."""
    output_dir = Path(output_dir)
    
    with open(output_dir / 'volcano_plot_instructions.txt', 'w') as f:
        f.write("="*80 + "\n")
        f.write("VOLCANO PLOT ANALYSIS INSTRUCTIONS\n")
        f.write("="*80 + "\n\n")
        
        f.write("FILE TO USE:\n")
        f.write("motif_occurrence_table_averaged_normalized.csv\n\n")
        
        f.write("WHAT THIS TABLE CONTAINS:\n")
        f.write("- Motif occurrences normalized by successful replicates per cell type\n")
        f.write("- Averaged across 4 model replicates (HumanBorzoi_rep0-3)\n")
        f.write("- Accounts for failed evolution runs with low specificity\n")
        f.write("- Ready for cell-type specificity analysis\n\n")
        
        f.write("TABLE FORMAT:\n")
        f.write("- Rows: Motifs\n")
        f.write("- Columns: Cell types + Total\n")
        f.write("- Values: Average normalized occurrence counts\n\n")
        
        f.write("EXAMPLE USAGE WITH YOUR VOLCANO PLOT CODE:\n")
        f.write("CSV_PATH = 'motif_occurrence_table_averaged_normalized.csv'\n\n")
        
        f.write("INTERPRETATION:\n")
        f.write("- Higher values = motif more enriched in that cell type\n")
        f.write("- Values account for different success rates across cell types\n")
        f.write("- Volcano plots will show cell-type-specific motif enrichment\n\n")
        
        f.write("KEY STATISTICS:\n")
        f.write(f"- Motifs analyzed: {len(averaged_table)}\n")
        f.write(f"- Cell types: {len(averaged_table.columns) - 1}\n")  # -1 for Total column
        f.write(f"- Value range: {averaged_table.drop('Total', axis=1).values.min():.3f} - {averaged_table.drop('Total', axis=1).values.max():.3f}\n")
        
        # Show top cell types by total normalized occurrences
        cell_totals = averaged_table.drop('Total', axis=1).sum().sort_values(ascending=False)
        f.write(f"\nTOP 10 CELL TYPES BY TOTAL NORMALIZED OCCURRENCES:\n")
        for i, (cell_type, total) in enumerate(cell_totals.head(10).items(), 1):
            f.write(f"{i:2d}. {cell_type}: {total:.1f}\n")
        
        # Show top motifs by total normalized occurrences
        f.write(f"\nTOP 10 MOTIFS BY TOTAL NORMALIZED OCCURRENCES:\n")
        top_motifs = averaged_table.nlargest(10, 'Total')
        for i, (motif, row) in enumerate(top_motifs.iterrows(), 1):
            f.write(f"{i:2d}. {motif}: {row['Total']:.1f}\n")
    
    print("✓ Volcano plot instructions saved")
